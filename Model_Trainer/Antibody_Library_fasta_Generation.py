...
from Dataset.tools.data_proc import Antibody_SecondaryStructure,select_model,Antibody_amino_acids_fasta

parser = argparse.ArgumentParser(description='Demo of argparse')
parser.add_argument('--generate_num',type=int ,default= 30)
parser.add_argument('--top_num',type=int ,default= 10)
parser.add_argument('--model',default= "BERT2DAb")
parser.add_argument('--target_library_path',default="Dataset/TargetAntibody_Library.fasta")
...
with open(f"./Result/{args.id}/TargetAntibody.txt","w") as w:
    for seq_record in SeqIO.parse(args.target_library_path, "fasta"):
...
if args.model =="BERT2DAb":
    ...
    Antibody_amino_acids_fasta(f"./Result/{args.id}/TargetAntibody.txt",f"./Result/{args.id}/TargetAntibody_Tokenizer.txt")
...
dataset_train = AbGAN_Dataset(sequence = sequences_path_train,
...
fasta_num = 0
max_sequences = args.generate_num * fasta_len
sequence_list = []  
...
with open(f"./Result/{args.id}/TargetAntibody_Library_{args.id}.fasta", 'w') as w:
    for sequence, label in dataloader_train:
        ...
        for i in range(int(batch_size)):
            ...
            if duplicate_index is not None:
                sequence_list[duplicate_index] = {"sequence": seq_1}
                continue
            else:
                sequence_list.append({"sequence": seq_1})
                fasta_num += 1
    ...
    sorted_sequences = sorted(sequence_count.items(), key=itemgetter(1), reverse=True)[:args.top_num*fasta_len]
